...
- [x] 影像尺寸、bit depth 與掃描序列完整性檢查。
- [x] 漏線／重複線、掉幀／中斷：優先讀取 acquisition log 或 encoder/timestamp 證據；無證據時不得僅靠單張影像宣稱為 0。
- [x] 拼接接縫亮度差與位置誤差。
- [x] 拼接不可檢區寬度；任一 ≥1 px 直接 S0。
- [x] 相機間灰階一致性。
- [x] 接縫造成不可檢區時，不受數值門檻影響直接 S0。
...
- [x] 缺陷 CNR；亮/暗兩種極性分別量測並取較差者。
- [x] Golden NG 檢出率、最差 margin、重拍次數與穩定漏檢。
- [x] 每型態 NG 樣本數與缺陷型態涵蓋率。
- [x] Golden PASS 誤檢率，要求 PASS N≥200 才可判 S3/S2。
- [x] 95% 單側 Clopper–Pearson 誤檢率上界。
- [x] 中心、兩側、拼接區的全寬檢出一致性。
- [x] 任一穩定漏檢、最小缺陷不可辨識或有效寬度 <2 px 觸發 S0 優先事件。
...
- [x] 依 G1～G6 結果產生主責單位與建議改善順序。
- [x] G1～G5 任一 S1/S0 時，報告明示 L2 不得作為通過對策。
- [x] 實作第 14.1 節 S2 與 G6 的關聯矩陣，但結果只作「推定/待確認」，不可自動取代三方確認與實驗證據。
- [x] 支援技術異議、三方意見與附錄 B 診斷實驗附件。
- [x] 新增 log-log 單一變因分析：至少 5 亮度點、每點 N≥30、分開 spatial STD 與 temporal noise，輸出迴歸圖與指數 b。

### P5.2 Waiver 工作流

- [x] 建立 waiver 資料模型與必要欄位、核准層級、責任人、硬體改善日與原判定。
- [x] 有效期限不得超過 90 日；到期自動失效並回復原判定。
- [x] 同一指標最多展延兩次，後續核准提高一階；第三次要求替代方案。
- [x] waiver 期間新增 S0 時立即失效。
- [x] 每 30 日追蹤進度、漏誤檢實績、最新指標與新增 S1/S0。
- [x] waiver 永遠不把原結果改寫成「驗收通過」。

### P5.3 《取像品質驗收報告》

- [x] 建立結構化報告模型，涵蓋第 16.1 節九章內容。
- [x] 輸出 HTML/PDF 與 machine-readable JSON；CSV 只作明細，不作唯一正式報告。
- [x] 報告逐項列出實測值、S 等級、群組、公式、ROI、日期、證據、規範版本與判定順位。
- [x] 附原始影像/參數/腳本/雜湊清單，確保每項分級可重算。
- [x] 支援取像主責、軟體、品質三方會簽與並列不同意見。
- [x] 清楚標示「內部工程管制線」，不宣稱為 ISO 強制門檻。
